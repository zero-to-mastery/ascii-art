...
DEFAULT_OUTPUT_TEXT_FILE = 'output.txt'
DEFAULT_OUTPUT_IMAGE_FILE = 'output.jpg'
...
def is_image_file(path_to_file):
    """
    This function checks if the file is a valid image.
    @param path_to_file: Path to the file to be checked
    @return: Boolean flag indicating if the path is valid or not
    """
...
def scale_image(image, new_width=100):
    ...
    @param new_width: The new width required; by default, it is 100
...
def map_pixels_to_ascii_chars(image, make_silhouette=False, range_width=25, brightness=1.0):
    ...
    @param make_silhouette: Flag to indicate if the image required to be a silhouette
...
def fetch_image_from_url(url):
    ...
    if response.status_code == 200:
        try:
            ...
            print("There is a problem when fetching an image from the URL")
            return e
    else:
...
def handle_image_conversion(image_filepath, url, make_silhouette=False, output_file_path=DEFAULT_OUTPUT_TEXT_FILE, brightness=1.0, output_image=False, custom_chars=None):
    ...
    try:
        ...
        print(f"Unable to open the image file {image_filepath}.")
...
def save_ascii_art_to_jpg(image_ascii, image):
    ...
    if len(image_ascii) <= 0:
        raise Exception('ASCII art string is of an invalid length')
    ...
    try:
        ...
        # Draw the text on the blank image from the top left corner with a font color of white
        ...
        resized_image.save(DEFAULT_OUTPUT_IMAGE_FILE)
        print(f'ASCII art image saved to {DEFAULT_OUTPUT_IMAGE_FILE}')
    except Exception as exception:
        raise exception
...
class SimpleCmd(cmd.Cmd):
    """
    This is a command interpreter class
    """
    prompt = "(hackfest) "
    ...
    def helf_quit(self):
        """
        This is the quit method help message
        """
    ...
    def do_ascii_text(self, args):
        """
        Converts text to an image and then to ASCII art
...
if __name__ == '__main__':
    ...
    print("Note! If you type '2', make sure you have 'custom_text.txt' file already in the root directory with ASCII-Art in it.")

    answer = input("Please type either '1' or '2': ")

    if answer == '2':
        if os.path.isfile(f'./{DEFAULT_OUTPUT_TEXT_FILE}'):
            art_to_image(DEFAULT_OUTPUT_TEXT_FILE)
        else:
            print("You did not create 'custom_text.txt' file in the home directory. Program ends here.")
        exit()
    ...
    parser.add_argument("-b", "--brightness", help="Alter brightness of the image (e.g. -b 1.0)", required=False)
    parser.add_argument("-l", "--url", help="Link to the image's URL on the internet")
    ...
    if args.interactive:
        ...
        handle_image_conversion(file_name, args.url,
                                ...
                                output_file_path=args.output if args.output else DEFAULT_OUTPUT_TEXT_FILE,
                                brightness=float(args.brightness) if args.brightness else 1.0,
                                output_image=args.output_image,
                                custom_chars=ASCII_CHARS)
