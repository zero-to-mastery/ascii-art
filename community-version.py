    ...
    for i, line in enumerate(frame.splitlines()):
        ...
        for j, char in enumerate(line):
            ...
            if colorize:
                ...
                draw.text((x_offset + (max_char_width - char_width) // 2, y_offset + (max_char_height - char_height) // 2), char, fill=pixel_color, font=font)
            else:
                draw.text((x_offset + (max_char_width - char_width) // 2, y_offset + (max_char_height - char_height) // 2), char, fill="black", font=font)
    ...
    if uploaded_file:
        ...
        if image.format == "GIF":
            ...
            if colorize:
                ...
                images = [convert_ascii_to_new_image_with_colors(ascii_frame, pattern_type, frame_color, colorize) for ascii_frame, frame_color in zip(ascii_frames, frame_colors)]
            else:
...
def run_cli(input_image: str, output: str, pattern_type: str, brightness: float, contrast: float, blur: bool, sharpen: bool, colorize: bool, theme: str, apply_contours: bool):
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        ...
        run_cli(args.input_image, args.output, args.pattern, args.brightness, args.contrast, args.blur, args.sharpen, args.colorize, args.theme, args.contours)
    else:
        run_streamlit_app()
